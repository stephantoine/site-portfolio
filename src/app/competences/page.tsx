...
const Competences = () => {
  ...
  const [constraints, setConstraints] = useState<{left:number,right:number,top:number,bottom:number}>({
  ...
  // marge = combien dépasse l’image quand elle est zoomée
  const overflowX = rect.width
  const overflowY = rect.height
  ...
  const Popup = () => (
    ...
    <div className="fixed inset-0 bg-black/60 z-40 flex items-center justify-center p-4">
      <div
        className="
          bg-white dark:bg-dark rounded-xl shadow-lg z-50
          max-w-5xl w-full
          max-h-[90vh]      /* limite verticale propre */
          flex flex-col
          overflow-hidden   /* empêche les débordements mobiles */
        "
      >
